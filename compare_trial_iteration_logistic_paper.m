...
n_test  = 10000;           % # testing samples


% Algorithm parameters
max_iter     = 30;
alpha        = 0.1;      % RGD stepsize
...
for trial = 1:num_trials
    ...
    prob = 1./(1 + exp(-logits));   % sigmoid函数
    y_train = binornd(1, prob);     % 二值标签 {0,1}
    ...
    for iter = 1:max_iter
        ...
        vec_G_current = vec_G_current - alpha * vec_grad_G; % GD
        ...
        vec_G_current_muon = vec_G_current_muon - alpha * vec_grad_G_muon; % GD
...
%%
%save compare_trial_iteration_time_logistic_112510.mat
...
% load ("compare_trial_iteration_time_logistic_120515_paper.mat")
...
hold on;
% LSRTR
lower_loss = max(mean_loss - std_loss, 1e-10);
...
% exportgraphics(gcf, fullfile(figs_path, 'loss_vs_iter_logistic.pdf'), 'ContentType', 'vector');
...
fill([iters, fliplr(iters)], [upper_errB; flipud(mean_errB - std_errB)]', ...
     'b', 'FaceAlpha', 0.2, 'EdgeColor', 'none', 'HandleVisibility', 'off');
...
% exportgraphics(gcf, fullfile(figs_path, 'estimation_vs_iter_logistic.pdf'), 'ContentType', 'vector');
...
% exportgraphics(gcf, fullfile(figs_path, 'prediction_vs_iter_logistic.pdf'), 'ContentType', 'vector');
...
% exportgraphics(gcf, fullfile(figs_path, 'loss_vs_time_logistic.pdf'), 'ContentType', 'vector');
...
% exportgraphics(gcf, fullfile(figs_path, 'estimation_vs_time_logistic.pdf'), 'ContentType', 'vector');
...
% exportgraphics(gcf, fullfile(figs_path, 'prediction_vs_time_logistic.pdf'), 'ContentType', 'vector');
